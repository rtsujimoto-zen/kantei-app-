...
短い文で、テンポよく、読みやすく構成してください。"""
        format_instruction = """
【出力フォーマット】
以下の構成で、温かく励ますように伝えてください。Markdownフォーマットで出力してください。

1. **ひと言であなたを表すと** - 「あなたは〇〇な人です！」と結論をズバッと伝える。キャッチーで覚えやすい一文にする

2. **あなたの魅力TOP3** - この人の最大の強みを3つ、箇条書きで。難しい言葉は使わず「〜できる力がある」「〜が得意」のような表現で

3. **心のエネルギータイプ** - エネルギーの強さを「パワフル型」「コツコツ型」「バランス型」のような直感的な表現で伝える。数値は使ってもOKだが、わかりやすい例えを添える

4. **人間関係のコツ** - 対人関係での傾向と、うまくいくためのアドバイスを具体的に

5. **今のあなたへのメッセージ** - 現在の運気を踏まえて、「今はこうするといい時期です」と具体的にアドバイス

6. **未来への応援メッセージ** - 背中を押すような、前向きで力強い一言

**重要**: 
- 専門用語は絶対に使わない（使う場合は「占いの世界では〇〇と呼ばれる」程度に留める）
- 文章は短く、テンポよく
- **太字**や絵文字を効果的に使い、親しみやすくする
